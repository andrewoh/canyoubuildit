...
export function PlayoffDepthChart() {
  ...
  return <figure className="sports-figure" aria-labelledby={`${id}-heading`}>
    <div className="sports-figure-head">
      ...
      <ExportButton svg={svg} title={title} subtitle={`${subtitle} Open dots = run year; red dots = following year. Red intervals = 95% confidence intervals for next-year means. 256 follow-ups, 2018–2025.`} filename="playoff-depth-and-next-season-injuries.svg" />
    </div>
    <div className="sports-key" aria-label="Chart legend">
      ...
      <span><i style={{ background: RED }} />Following season</span>
      <span>Thin red interval = 95% confidence interval for the following-season mean</span>
    </div>
      ...
      <svg ref={svg} className="sports-chart" style={chartStyle} viewBox="0 0 800 414" role="img" aria-labelledby={`${id}-title ${id}-desc`}>
        ...
        {rows.map((row, i) => {
          ...
          return <g key={row.category}>
            ...
            <Text x={0} y={y + 17}>{row.n} follow-up seasons</Text>
            <line x1={x(row.prior_agl_index)} x2={x(row.next_agl_index)} y1={y} y2={y} stroke={PRIOR} strokeWidth={2} />
            <line x1={x(row.mean_lo)} x2={x(row.mean_hi)} y1={y + 14} y2={y + 14} stroke={RED} strokeWidth={1.5} opacity={0.75} />
            <line x1={x(row.mean_lo)} x2={x(row.mean_lo)} y1={y + 10} y2={y + 18} stroke={RED} />
            <line x1={x(row.mean_hi)} x2={x(row.mean_hi)} y1={y + 10} y2={y + 18} stroke={RED} />
            <circle cx={x(row.prior_agl_index)} cy={y} r={5.5} fill={PAPER} stroke={PRIOR} strokeWidth={2} />
            <circle cx={x(row.next_agl_index)} cy={y} r={6} fill={RED} stroke={PAPER} strokeWidth={1.5} />
        ...
        <Text x={510} y={405} textAnchor="middle">Injury index · higher means more injury burden</Text>
      </svg>
    </div>
    ...
    <details className="sports-view-table"><summary>View data and uncertainty</summary><div className="sports-table-scroll"><table className="sports-table">
      ...
      <thead><tr><th scope="col">Previous season’s finish</th><th scope="col">Follow-ups</th><th scope="col">During run</th><th scope="col">Next season</th><th scope="col">Next-season mean 95% CI</th></tr></thead>
      <tbody>{rows.map(row => <tr key={row.category}><th scope="row">{row.category}</th><td>{row.n}</td><td>{fmt(row.prior_agl_index)}</td><td>{fmt(row.next_agl_index)}</td><td>{fmt(row.mean_lo)} to {fmt(row.mean_hi)}</td></tr>)}</tbody>
    </table></div></details>
  </figure>;
}
...
export function SuperBowlChart() {
  ...
  return <figure className="sports-figure" aria-labelledby={`${id}-heading`}>
    <div className="sports-figure-head">
      ...
      <ExportButton svg={svg} title={title} subtitle={`${subtitle} Red = SF. Filled dots = next season; open dots = Super Bowl season when shown. 16 observations from seven franchises.`} filename="all-super-bowl-injury-followups.svg" />
    </div>
    ...
    <div className="sports-key" aria-label="Chart legend">
      <span><i style={{ background: RED }} />San Francisco</span><span><i style={{ background: INK }} />Other teams</span>
    ...
    <div className="sports-chart-scroll" tabIndex={0} role="region" aria-label="Ranked Super Bowl follow-ups; scroll horizontally on small screens">
      <svg ref={svg} className="sports-chart" style={chartStyle} viewBox={`0 0 800 ${height}`} role="group" aria-labelledby={`${id}-title ${id}-desc`}>
        ...
        {rows.map((row, i) => {
          ...
          const color = row.team === "SF" ? RED : INK;
          return <g key={key} className="plot-target" role="button" tabIndex={0} aria-pressed={active}
            aria-label={`${row.team}, ${row.season}: injury index ${fmt(row.agl_index)}, following the ${row.exposure_season} Super Bowl season. Select for details.`}
            ...
            <Text x={0} y={y + 5} style={{ fill: color, fontWeight: row.team === "SF" || active ? 700 : 400 }}>{row.team} · {row.season}</Text>
            {showPrior && <><line x1={x(row.prior_agl_index)} x2={x(row.agl_index)} y1={y} y2={y} stroke={PRIOR} strokeWidth={1.6} /><circle cx={x(row.prior_agl_index)} cy={y} r={4.5} fill={PAPER} stroke={PRIOR} strokeWidth={1.7} /></>}
        ...
        <Text x={466} y={721} textAnchor="middle">Next-season injury index · higher means more injury burden</Text>
      </svg>
    </div>
    <div className="sports-chart-detail" aria-live="polite" aria-atomic="true"><strong>{teamName}, {selected.exposure_season} → {selected.season}.</strong>{" "}Injury index {fmt(selected.prior_agl_index)} → {fmt(selected.agl_index)} ({signed(selected.delta_index)} points). The team played {selected.prior_post_games} postseason games; the following regular season recorded {fmt(selected.agl)} adjusted games lost.</div>
    ...
    <details className="sports-view-table"><summary>View all 16 follow-up seasons</summary><div className="sports-table-scroll"><table className="sports-table">
      ...
      <tbody>{rows.map(row => <tr key={`${row.team}-${row.season}`}><th scope="row">{row.team}</th><td>{row.exposure_season}</td><td>{row.season}</td><td>{fmt(row.prior_agl_index)}</td><td>{fmt(row.agl_index)}</td><td>{signed(row.delta_index)}</td><td>{row.prior_post_games}</td></tr>)}</tbody>
    </table></div></details>
  </figure>;
}
...
export function EffectChart() {
  ...
  return <figure className="sports-figure" aria-labelledby={`${id}-heading`}>
    ...
    <div className="sports-key"><span><i style={{ background: RED }} />Point estimate</span><span>Horizontal line = <Metric term="interval">95% uncertainty range</Metric></span></div>
    <p className="sports-scroll-hint">Swipe the chart horizontally to see the full comparison →</p>
      ...
      <svg ref={svg} className="sports-chart" style={chartStyle} viewBox="0 0 800 407" role="img" aria-labelledby={`${id}-title ${id}-desc`}>
        ...
        {rows.map((row, i) => {
          ...
          return <g key={row.feature}>
            ...
            <Text x={0} y={y + 36} style={{ fill: RED }}>{signed(row.coef)} ({signed(row.lo)} to {signed(row.hi)})</Text>
            ...
            <circle cx={x(row.coef)} cy={y + 4} r={6} fill={RED} stroke={PAPER} strokeWidth={1.5} />
          </g>;
        })}
        <Text x={335} y={395}>Less injury burden ←</Text><Text x={780} y={395} textAnchor="end">→ More injury burden</Text>
      </svg>
    </div>
