...
const RED = '#b33127', TEAL = '#246b5a', INK = '#22251f', MUTED = '#63675e', GRAY = '#a9aca2', LIGHT = '#d8d7ce';
const WIDTH = 800;
...
const burdenColor = (value: number) => value > 100.05 ? RED : value < 99.95 ? TEAL : MUTED;
const burdenLabel = (value: number) => Math.abs(value - 100) < .05 ? 'At the NFL average' : `${value > 100 ? '↑' : '↓'} ${num(Math.abs(value - 100))}% ${value > 100 ? 'more' : 'less'} injury burden than the NFL average`;
...
export function LeagueChart() {
  ...
  return <Figure number="01" exportNotes={mode === 'annual' ? 'The red line identifies the selected team; point and value colors show injury burden: red above the NFL average, teal below. Gray lines show other teams. The dashed line is 100, the league average.' : 'Bars and values are red above the NFL average and teal below; the dark-outlined bar identifies the selected team. The dashed line is 100, the league average.'} title="Where each team sits in the injury table" subtitle="Injury burden relative to that season’s league. Higher is worse; 100 is the NFL average." source="Sources: Football Outsiders / FTN adjusted games lost, 2017–2025. Era ranking averages annual indexes equally. Excluding 2020–21 tests pandemic-era sensitivity; direct COVID absences were already excluded from those AGL tables.">
    ...
    <div className="sports-key"><TeamBadge team={team} /><span><i style={{ background: GRAY }} />Other NFL teams</span><span>Dashed line: 100 = league average</span></div>
    <BurdenComparison value={mode === 'annual' ? detail.index : mean} team={team} period={mode === 'annual' ? String(detail.season) : omit ? 'Seven selected seasons' : '2017–2025 average'} />
    <DirectionKey kind="burden" />
    {mode === 'annual' ? <Svg height={386} label={`${names[team]} annual injury index${omit ? ', excluding 2020–2021' : ''}`}>
      <SvgTeamLogo team={team} x={70} y={11} size={24} /><text x={101} y={28} style={{ fontWeight: 700 }}>{names[team]}</text><text x={730} y={28} textAnchor="end" style={{ fill: RED, fontWeight: 700 }}>↑ Higher = more injury burden</text>
      ...
      {selected.map(r => <g key={r.season} className="plot-target" tabIndex={0} role="button" aria-label={`${r.season}: injury index ${num(r.index)}. ${burdenLabel(r.index)}`} onMouseEnter={() => setDetailYear(r.season)} onFocus={() => setDetailYear(r.season)} onClick={() => setDetailYear(r.season)} onKeyDown={e => onKey(e, () => setDetailYear(r.season))}><circle cx={xYear(r.season)} cy={y(r.index)} r="12" fill="transparent" /><circle cx={xYear(r.season)} cy={y(r.index)} r={detail.season === r.season ? 6 : 4} fill={burdenColor(r.index)} stroke="#f6f3ec" strokeWidth="2" /><text x={xYear(r.season)} y={y(r.index) - 12} textAnchor="middle" style={{ fill: burdenColor(r.index), fontWeight: 700 }}>{num(r.index, 0)}</text></g>)}
      {years.map(t => <text key={t} x={xYear(t)} y={327} textAnchor="middle">{t}</text>)}
      <text x={70} y={365} style={{ fill: burdenColor(detail.index), fontWeight: 700 }}>{detail.season}: {burdenLabel(detail.index)}</text>
    </Svg> : <Svg height={893} label={`NFL era injury ranking${omit ? ', excluding 2020–2021' : ', 2017–2025'}`}>
      <text x={xr(0)} y={20} style={{ fill: TEAL, fontWeight: 700 }}>← Less injury burden</text><text x={709} y={20} textAnchor="end" style={{ fill: RED, fontWeight: 700 }}>More injury burden →</text>
      <Grid ticks={[0, 50, 100, 150]} x={xr} y1={41} y2={808} />
      {ranking.map((r, i) => <g key={r.team} className="plot-target" tabIndex={0} role="button" aria-label={`${names[r.team]}, rank ${i + 1}, injury index ${num(r.index)}. ${burdenLabel(r.index)}`} onClick={() => setTeam(r.team)} onKeyDown={e => onKey(e, () => setTeam(r.team))}><SvgTeamLogo team={r.team} x={9} y={38 + i * 24} size={21} /><text x={69} y={54 + i * 24} textAnchor="end" style={{ fill: r.team === team ? INK : MUTED, fontWeight: r.team === team ? 700 : 400 }}>{r.team}</text><rect x={xr(0)} y={42 + i * 24} width={xr(r.index) - xr(0)} height="15" fill={burdenColor(r.index)} opacity={r.team === team ? 1 : .48} stroke={r.team === team ? INK : 'none'} strokeWidth={1.5} /><text x={xr(r.index) + 8} y={54 + i * 24} style={{ fill: burdenColor(r.index), fontWeight: r.team === team ? 700 : 400 }}>{num(r.index)}</text></g>)}
      <text x={83} y={861} style={{ fill: burdenColor(mean), fontWeight: 700 }}>{team}: {burdenLabel(mean)}</text>
    </Svg>}
    <div className="sports-chart-detail" aria-live="polite"><TeamBadge team={team} compact />: {num(mean)} across {omit ? 'seven selected' : 'nine'} seasons, rank {rank} of 32 from most injured. {mode === 'annual' && <>{detail.season}: {num(detail.index)} ({num(detail.agl)} adjusted games lost).</>}</div>
...
export function ProfilesChart() {
  ...
  return <Figure number="06" exportNotes={`Map markers: filled gray = established contenders; hollow gray = less established rosters; team logo with red outline = selected team (identity, not health). Fingerprint dots are neutral: higher percentiles mean more of an attribute, not more injury risk. In the injury comparison, red bars mean more burden than the NFL average; teal bars mean less. The dark-outlined bar identifies the selected team. Features describe the ${selected.feature_season} regular season; playoff exposure spans ${year - 3}–${year - 1}. Continuity is the share of ${selected.feature_season} snaps played by players who also recorded a same-club scrimmage snap in ${year - 2}; it does not measure retention into ${year}. Percentile fingerprints compare all 32 clubs entering ${year}.`} title="How similar teams fared" subtitle="Choose a season and team to explore broad roster profiles, then compare the five closest teams in all six measured attributes." source="Sources: nflverse snaps, schedules, player birth dates and team statistics; Football Outsiders / FTN AGL. All attributes precede the injury season. Injury index: 100 = that season’s NFL average. Injury outcomes were excluded from clustering. Nearest profiles use all six standardized attributes; they are comparisons, not causal controls.">
    ...
    <div className="sports-key"><TeamBadge team={team} label={`${names[team]} · selected`} /><span><i style={{ background: MUTED }} />Established contenders</span><span><i style={{ background: 'transparent', border: `1.5px solid ${MUTED}` }} />Less established rosters</span></div>
    ...
    <div className="sports-chart-detail" aria-live="polite"><TeamBadge team={team} compact /> <strong>in {year}</strong> falls in “{clusterName.toLowerCase()}.” Broad-group membership is retained in {num(selected.membership_stability * 100, 0)}% of franchise bootstrap fits. Its injury index is <strong>{num(selected.index)}</strong>; 100 is the league average.</div>
    ...
    <DirectionKey kind="profile" />
    <Svg height={307} label={`${names[team]} entering ${year}: six-attribute percentile fingerprint`}>
    ...
    <BurdenComparison value={selected.index} team={team} period={String(year)} />
    <DirectionKey kind="burden" />
    <Svg height={338} label={`${year} injury burden: ${names[team]} and its five closest profiles (100 = NFL average)`}>
      <text x={peerX(0)} y={19} style={{ fill: TEAL, fontWeight: 700 }}>← Less injury burden</text><text x={700} y={19} textAnchor="end" style={{ fill: RED, fontWeight: 700 }}>More injury burden →</text>
      <Grid ticks={Array.from({ length: maxBurden / 50 + 1 }, (_, i) => i * 50)} x={peerX} y1={42} y2={258} />
      {displayPeers.map((r, i) => <g key={r.team}><title>{`${names[r.team]}: ${burdenLabel(r.index)}`}</title><SvgTeamLogo team={r.team} x={27} y={47 + i * 35} size={25} /><text x={104} y={66 + i * 35} textAnchor="end" style={{ fill: INK, fontWeight: r.team === team ? 700 : 400 }}>{r.team}{i > 0 ? ` ·${i}` : ''}</text><rect x={peerX(0)} y={52 + i * 35} width={peerX(r.index) - peerX(0)} height={19} fill={burdenColor(r.index)} opacity={r.team === team ? 1 : .65} stroke={r.team === team ? INK : 'none'} strokeWidth={1.5} /><text x={peerX(r.index) + 8} y={66 + i * 35} style={{ fill: burdenColor(r.index), fontWeight: 700 }}>{num(r.index)}</text></g>)}
      <text x={120} y={315} style={{ fill: burdenColor(selected.index), fontWeight: 700 }}>{team} in {year}: {burdenLabel(selected.index)}</text>
    </Svg>
