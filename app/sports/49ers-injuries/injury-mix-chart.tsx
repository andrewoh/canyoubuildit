...
export function InjuryMixChart() {
  ...
  const exportNotes = `${source} Each physical player-game listing contributes one unit, divided among its different body regions. Repeat weekly reports count again. Other 31 shares are matched to the selected team's season and listed-position mix. Arrows indicate a higher or lower share than the comparison, not better or worse health. ${mode === 'shares' ? 'Red dot: selected team. Gray dot: matched other 31 teams.' : 'Red dot: selected team minus matched other 31 teams.'} ${primary && mode === 'shares' ? 'The approximate simultaneous 95% ranges for differences across all 11 categories include zero; ranges are not shown in this share view.' : intervalNote} Knee does not identify ACL tears; Calf/Achilles does not identify ruptures.`;
  ...
  return <figure className="sports-figure" ref={figure}>
    ...
    <div className="sports-key"><span><i style={{ background: RED }} /><TeamBadge team={team}/></span>{mode === 'shares' ? <span><i style={{ background: GRAY }} />Matched other 31 teams</span> : <span>{primary ? 'Whiskers: simultaneous 95% ranges across 11 categories' : 'Descriptive differences · no uncertainty ranges'}</span>}</div>
    <ShareComparison team={team} period={period} label={`${category} share`} difference={selected.adjusted_difference_pp} reference="season/position-matched other teams"/><DirectionKey kind="share"/><p className="sports-scroll-hint">Swipe to explore the full chart →</p>
    <div className="sports-chart-scroll"><svg className="sports-chart" viewBox="0 0 800 600" style={{ minWidth: 700 }} role="group" aria-label={`${names[team]}, ${period}: ${mode === 'shares' ? 'body-region report shares' : 'differences in report share'}`}><title>{`${names[team]}, ${period}: physical injury-report body-region mix`}</title>
      ...
      <SvgTeamLogo team={team} x={216} y={5} size={24}/><text x={248} y={23} className="chart-strong">{names[team]}</text>
      ...
      {rows.map((row, index) => { const cy = 62 + index * 44, active = row.category === category, ci = interval(row.category); return <g key={row.category} className="plot-target" role="button" tabIndex={0} aria-pressed={active} aria-label={`${row.category}: ${fmt(row.share * 100)}% for ${names[team]}, ${fmt(row.other_standardized_share * 100)}% for matched other 31 teams; difference ${sign(row.adjusted_difference_pp)} percentage points. Show listing details.`} onClick={() => setCategory(row.category)} onKeyDown={e => activate(e, () => setCategory(row.category))}>
        ...
        {mode === 'shares' ? <><line x1={x(row.share * 100)} x2={x(row.other_standardized_share * 100)} y1={cy} y2={cy} stroke={GRAY} strokeWidth={2} /><circle cx={x(row.other_standardized_share * 100)} cy={cy} r={5.5} fill={GRAY} stroke="#f6f3ec" strokeWidth={1.5} /><circle cx={x(row.share * 100)} cy={cy} r={5.5} fill={RED} stroke="#f6f3ec" strokeWidth={1.5} /><text x={707} y={cy + 4} textAnchor="end" className="chart-red">{`${Math.abs(row.adjusted_difference_pp)<.05?'≈':row.adjusted_difference_pp>0?'↑':'↓'} ${fmt(row.share * 100)}%`}</text><text x={780} y={cy + 4} textAnchor="end">{fmt(row.other_standardized_share * 100)}%</text></> : <>{primary && <><line x1={x(ci.simultaneous_lo_pp)} x2={x(ci.simultaneous_hi_pp)} y1={cy} y2={cy} stroke={GRAY} strokeWidth={2} /><line x1={x(ci.simultaneous_lo_pp)} x2={x(ci.simultaneous_lo_pp)} y1={cy - 5} y2={cy + 5} stroke={GRAY} /><line x1={x(ci.simultaneous_hi_pp)} x2={x(ci.simultaneous_hi_pp)} y1={cy - 5} y2={cy + 5} stroke={GRAY} /></>}<circle cx={x(row.adjusted_difference_pp)} cy={cy} r={5.5} fill={RED} /><text x={780} y={cy + 4} textAnchor="end" className="chart-red">{`${Math.abs(row.adjusted_difference_pp)<.05?'≈':row.adjusted_difference_pp>0?'↑':'↓'} ${sign(row.adjusted_difference_pp)}`}</text></>}
      </g>; })}
    ...
    <text x={216} y={589}>← Smaller share</text><text x={645} y={589} textAnchor="end">Larger share →</text>
    </svg></div>
